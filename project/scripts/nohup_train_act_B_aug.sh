...
# Manual launcher for ACT-B data-augmentation full-scale training.
...
CONFIG="$LEROBOT_SOURCE/project/configs/act_B_aug_full.yaml"
RUN_ID="$(date -u +%Y%m%d_%H%M%S)_act_B_aug_full_gpu${GPU_ID}"
RUN_DIR="$CALVIN_RUNS/act_B_aug/$RUN_ID"
LOG_DIR="$CALVIN_RUNS/act_B_aug/logs"
LOG_FILE="$LOG_DIR/${RUN_ID}.log"

mkdir -p "$RUN_DIR" "$LOG_DIR"

echo "Starting ACT-B data-augmentation full training manually."
echo "GPU_ID=$GPU_ID"
echo "CONFIG=$CONFIG"
...
echo "  PROGRESS act_B_aug_full [########------------------------] 25000/100000 ( 25.0%) elapsed=... eta=..."
echo
